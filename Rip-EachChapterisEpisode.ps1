...
For($i = 1; $i -le $numberOfTitles; $i++)
{
    ...
    For($j = 1; $j -le $numberOfChapters; $j++)
    {
        $outputAbsolutePath = $outputFilePath + $nameOfShow + "e" + $j + ".mp4"
        ...
        .\HandBrakeCLI.exe -t $i -i $inputDriveLetter -c $j -o $outputAbsolutePath --audio-lang-list "English" --ab 192  --all-subtitles -e nvenc_h265 --no-dvdnav  
    }
}
